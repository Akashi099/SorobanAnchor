...
pub struct OutboundRequestOptions {
    ...
    /// Credentials for the target endpoint. When `Some`, adds the
    /// corresponding authentication header (see [`RequestCredentials`]).
    pub credentials: Option<RequestCredentials>,
}

/// `Debug` is implemented manually so secrets never leak into logs: the
/// signing key and credentials are redacted, only their presence is shown.
...
impl OutboundRequestOptions {
    ...
    /// Attach [`RequestCredentials`] to this options set.
    ...
    pub fn with_bearer_token(self, token: impl Into<String>) -> Self {
        self.with_credentials(RequestCredentials::Bearer(token.into()))
    }

    /// Attach HTTP Basic credentials, sent as `Authorization: Basic <base64>`.
    pub fn with_basic_auth(self, username: impl Into<String>, password: impl Into<String>) -> Self {
        self.with_credentials(RequestCredentials::Basic {
    ...
    /// - Authentication header — when `credentials` is set
    ///   (see [`RequestCredentials::to_header`]).
